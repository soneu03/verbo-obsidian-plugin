...
var VerboSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    containerEl.createEl("h2", { text: "Configuraci\xF3n de Verbo" });
    new import_obsidian.Setting(containerEl).setName("API Key de Google Gemini").setDesc("Ingresa tu API key de Google Gemini para procesar las transcripciones").addText((text) => text.setPlaceholder("API Key").setValue(this.plugin.settings.geminiApiKey).onChange(async (value) => {
      this.plugin.settings.geminiApiKey = value;
      await this.plugin.saveSettings();
    }));
    containerEl.createEl("h3", { text: "Gesti\xF3n de Prompts" });
    new import_obsidian.Setting(containerEl).setName("Seleccionar prompt").setDesc("Elige un prompt predefinido o personalizado para procesar las transcripciones").addDropdown((dropdown) => {
    ...
    const isPredefined = (selectedPrompt == null ? void 0 : selectedPrompt.isDefault) || false;
    new import_obsidian.Setting(containerEl).setName("Nombre del prompt").setDesc("Edita el nombre del prompt seleccionado").addText((text) => {
      text.setValue((selectedPrompt == null ? void 0 : selectedPrompt.name) || "").setDisabled(isPredefined).onChange(async (value) => {
    ...
    const promptEditorHeader = new import_obsidian.Setting(promptEditorContainer).setName("Contenido del prompt").setDesc("Edita el contenido del prompt seleccionado");
    if (isPredefined) {
      promptEditorHeader.addButton((button) => button.setButtonText("Restaurar predeterminado").onClick(async () => {
        await this.plugin.resetPredefinedPrompt(this.plugin.settings.selectedPromptIndex);
        this.display();
        new import_obsidian.Notice("Prompt restaurado a su contenido predeterminado");
      }));
    }
    ...
    new import_obsidian.Setting(promptButtonsContainer).setName("Gestionar prompts").addButton((button) => button.setButtonText("A\xF1adir nuevo prompt").onClick(async () => {
      const newPrompt = {
        name: "Nuevo prompt",
        content: "Ingresa aqu\xED tu prompt personalizado:",
        isDefault: false
      };
      ...
      await this.plugin.saveSettings();
      this.display();
      new import_obsidian.Notice("Nuevo prompt a\xF1adido");
    })).addButton((button) => button.setButtonText("Eliminar prompt").setDisabled(isPredefined).onClick(async () => {
      if (isPredefined) {
        new import_obsidian.Notice("No se pueden eliminar los prompts predefinidos");
        return;
      }
      ...
      await this.plugin.saveSettings();
      this.display();
      new import_obsidian.Notice("Prompt eliminado");
    }));
    containerEl.createEl("h3", { text: "Otras configuraciones" });
    new import_obsidian.Setting(containerEl).setName("Incluir marcas de tiempo").setDesc("Incluir marcas de tiempo en la transcripci\xF3n original").addToggle((toggle) => toggle.setValue(this.plugin.settings.includeTimestamps).onChange(async (value) => {
      this.plugin.settings.includeTimestamps = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Mostrar transcripci\xF3n original").setDesc("Incluir la transcripci\xF3n original en la nota generada").addToggle((toggle) => toggle.setValue(this.plugin.settings.showOriginalTranscript).onChange(async (value) => {
...
async function getTranscriptNative(videoId, includeTimestamps = true) {
  ...
  try {
    ...
    const playerScript = scripts.find(
      (script) => script.textContent.includes("var ytInitialPlayerResponse = {")
    );
    if (!playerScript) {
      throw new Error("No se pudo encontrar la informaci\xF3n del video");
    }
    const dataString = ((_d = (_c = (_b = (_a = playerScript.textContent) == null ? void 0 : _a.split("var ytInitialPlayerResponse = ")) == null ? void 0 : _b[1]) == null ? void 0 : _c.split("};")) == null ? void 0 : _d[0]) + "}";
    const data = JSON.parse(dataString.trim());
    const availableCaptions = ((_f = (_e = data == null ? void 0 : data.captions) == null ? void 0 : _e.playerCaptionsTracklistRenderer) == null ? void 0 : _f.captionTracks) || [];
    if (!availableCaptions.length) {
      throw new Error("No se encontr\xF3 transcripci\xF3n para este video");
    }
    ...
    const resXML = await (0, import_obsidian2.request)(fixedCaptionsUrl).then(
      (xml) => (0, import_node_html_parser.parse)(xml)
    );
    const chunks = resXML.getElementsByTagName("text");
    let transcript = chunks.map((cue) => {
      const text = cue.textContent.replaceAll("&#39;", "'").replaceAll("&amp;", "&").replaceAll("&quot;", '"').replaceAll("&apos;", "'").replaceAll("&lt;", "<").replaceAll("&gt;", ">");
...
`;
    ...
    if (error.message.includes("token limit")) {
      throw new Error("La transcripci\xF3n es demasiado larga para ser procesada. Intenta con un video m\xE1s corto.");
    } else if (error.message.includes("quota")) {
      throw new Error("Se ha excedido la cuota de la API de Google Gemini. Intenta m\xE1s tarde.");
    } else {
...
var VerboResultView = class extends import_obsidian3.Modal {
  ...
  onOpen() {
    ...
    const originalTab = tabButtons.createEl("button", { text: "Transcripci\xF3n Original" });
    ...
    const insertButton = new import_obsidian3.ButtonComponent(buttonContainer).setButtonText("Insertar en nota actual").setCta().onClick(() => {
      const activeView = this.app.workspace.getActiveViewOfType(import_obsidian3.MarkdownView);
      if (activeView) {
        const editor = activeView.editor;
        const cursor = editor.getCursor();
        const contentArray = [
          `## ${this.result.videoTitle || "Transcripci\xF3n de YouTube"}`,
        ...
        if (this.plugin.settings.showOriginalTranscript) {
          contentArray.push(
            "---",
            "> [!info] Transcripci\xF3n original",
            "> ```",
        ...
        new import_obsidian3.Notice("No hay una nota abierta para insertar el contenido");
      }
    });
    const newNoteButton = new import_obsidian3.ButtonComponent(buttonContainer).setButtonText("Crear nueva nota").onClick(() => {
      this.createNote();
    });
    const closeButton = new import_obsidian3.ButtonComponent(buttonContainer).setButtonText("Cerrar").onClick(() => {
      this.close();
    });
  }
  onClose() {
  ...
  async createNote() {
    ...
    try {
      const activeFolder = this.app.vault.getAbstractFileByPath(
        ((_b = (_a = this.app.workspace.getActiveFile()) == null ? void 0 : _a.parent) == null ? void 0 : _b.path) || "/"
      );
      ...
      this.close();
      new import_obsidian3.Notice("Nota creada exitosamente");
    } catch (error) {
      console.error("Error al crear la nota:", error);
      new import_obsidian3.Notice(`Error al crear la nota: ${error.message}`);
    }
  }
...
// src/ui/modal.ts
var VerboModal = class extends import_obsidian4.Modal {
  constructor(app, plugin) {
    super(app);
    this.youtubeUrl = "";
  ...
  onOpen() {
    const { contentEl } = this;
    contentEl.createEl("h2", { text: "Verbo: Transcripci\xF3n de YouTube" });
    new import_obsidian4.Setting(contentEl).setName("URL de YouTube").setDesc("Ingresa la URL del video de YouTube").addText((text) => text.setPlaceholder("https://www.youtube.com/watch?v=...").onChange((value) => {
      this.youtubeUrl = value;
    }).inputEl.style.width = "100%");
    new import_obsidian4.Setting(contentEl).setName("Seleccionar prompt").setDesc("Elige el tipo de procesamiento para la transcripci\xF3n").addDropdown((dropdown) => {
      this.plugin.settings.customPrompts.forEach((prompt, index) => {
    ...
    const processButton = new import_obsidian4.ButtonComponent(buttonContainer).setButtonText("Procesar").setCta().onClick(async () => {
      ...
      if (!this.youtubeUrl) {
        new import_obsidian4.Notice("Por favor, ingresa una URL de YouTube v\xE1lida");
        return;
      }
    ...
    const cancelButton = new import_obsidian4.ButtonComponent(buttonContainer).setButtonText("Cancelar").onClick(() => {
      this.close();
    });
  }
  onClose() {
...
var VerboPlugin = class extends import_obsidian5.Plugin {
  async onload() {
    ...
    (0, import_obsidian5.addIcon)("verbo-icon", `<svg viewBox="0 0 100 100" width="100" height="100">
      <path fill="currentColor" d="M50 20 L75 40 L75 80 L25 80 L25 40 Z M40 50 L60 50 M40 60 L60 60 M40 70 L60 70"></path>
    </svg>`);
    this.addCommand({
      id: "open-verbo-modal",
      name: "Obtener transcripci\xF3n de YouTube",
      icon: "verbo-icon",
      callback: () => {
  ...
  async loadPredefinedPromptContent(promptPath) {
    try {
  ...
  async resetPredefinedPrompt(index) {
